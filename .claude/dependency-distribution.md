...
Go 里模块路径=类型身份,不能像 Rust 那样解耦。但用  可**重定向拉取位置、消费方 import 零改动**:
- hi-proto-code/go:(内部 import 也用 github,全场一个名), + 自 replace (独立编译用)。布局仍干净 。
- 消费方 go.mod(import 一个字不动):
  
  + GOPRIVATE=git.hi.lan + git insteadOf ssh。
