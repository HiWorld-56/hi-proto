...
**CI 打不打号,判据是 hi-proto-code 的生成物有没有变** —— 不是"推没推 dev"。
推基号(只动 `VERSION`)、只改 `ci/` `smoke/` 这类,生成物一个字节都不变,**不产号**。

2026-09-10 定的。之前一度是"每个 dev 提交都要有号",于是刚发完正式版 `v1.5.14`、
紧接着推基号,CI 就蹦出一个 `v1.5.15-dev.1`,而它在 hi-proto-code 那边与 `v1.5.14`
**指向同一个 commit** —— 同事看见新号只会以为"又更新了"。
**号的意义是"这里有一份新的生成物可以取"**;它一旦可以为空,新号就不再意味着任何事。

老规矩「没打 tag 的提交消费方根本够不着,等于没发布」没被推翻:它讲的是**够不着**,
而生成物没变时,钉上一个号拿到的就是一字不差的同一份代码。细节见 `ci/README.md`。
