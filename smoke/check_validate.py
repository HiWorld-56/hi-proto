...
REPOS = sys.argv[1:] or ['/home/lo/wip/backend-hi-club']
GOMODCACHE = subprocess.run(['go','env','GOMODCACHE'], capture_output=True, text=True,
                            env={**os.environ,'PATH':os.environ['PATH']+':/home/lo/golang/go/bin'}).stdout.strip()
...
def load_pb_fields(ver):
    ...
    d = f'{GOMODCACHE}/github.com/!hi!world-56/hi-proto@{ver}/go'
    pats = glob.glob(f'{d}/**/*.pb.go', recursive=True)
    if not pats:
        print(f'!! 找不到 {d} 下的 pb.go(该版本没下载?)'); sys.exit(2)
    for f in pats:
