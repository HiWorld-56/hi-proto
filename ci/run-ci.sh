...
# ── 打号的判据:**生成物变了才有新号** ─────────────────────────────────────────
#
# ⭐ 2026-09-10 改回内容判据(09-05 那次「每个 dev 提交都要有号」作废,理由如下)。
#
# 09-05 的口径是「推了 dev 就必须打 tag,一次都不能省」,于是**推基号**这种只动 VERSION
# 的提交也会出一个号:刚发完正式版 v1.5.14,紧接着蹦出一个 v1.5.15-dev.1,而它的生成物
# 与 v1.5.14 **是同一个 commit** —— 同事看见新号只会以为"又更新了",去 diff 才发现
# 一个字没改。**号的意义是"这里有一份新的生成物可以取"**;它一旦可以为空,
# 新号就不再意味着任何事,而"到哪个版本就是哪个版本"这件事也就没人信了。
#
# 那条老规矩(「没打 tag 的提交消费方根本够不着,等于没发布」)**没有被推翻** ——
# 它讲的是**够不着**。生成物没变时,消费方钉上一个号拿到的就是一字不差的同一份代码,
# 不存在够不着。两者本来不冲突:**判据是"生成物变没变",不是"推没推 dev"。**
#
# 判据用**内容**,不用路径:路径过滤只用来省掉那趟生成(整轮里最慢的一段);
# 真正决定打不打号的,是 release.sh 跑完之后 hi-proto-code 的 dev 有没有往前走 ——
# 它自己就有"产物无变更"那一档,什么都不提交。于是"改了 .proto 但生成物没变"
# (比如只动注释里不进生成物的那部分)也一并被这条判据兜住,路径过滤兜不住。
#
# 顺带把老问题也解决了:v1.5.0-dev.166/167/168 指向同一个生成物 commit c2faf3e4、
# v1.5.1 / -dev.1 / -dev.2 又同为 563f7e58 —— 这类"三个号一个东西"以后不会再产生。
#
# ⚠️ main(正式发布)**不受这条约束** —— 发版必须出号,哪怕这一版与上一个 dev 号一模一样。
...
if [ "$BR" != main ]; then
  ...
  if [ -z "$LAST_TAG" ]; then
    ...
    if [ -z "$GEN_CHANGED" ]; then
      ...
      SKIP_GEN=1
      echo "[ci] 自 $LAST_TAG 起没有影响生成物的改动 → 跳过生成(也就不会有新号,见下)。"
...
# 生成 + 推 hi-proto-code dev。跳过那一档只是省时间,不改变结论 ——
# 结论看的是下面这两个 SHA 一不一样。
CODE_BEFORE=$(git -C "$CODE" rev-parse dev)
if [ "${SKIP_GEN:-0}" = 1 ]; then
  echo "[ci] 跳过生成(路径过滤:没有影响生成物的改动)"
else
  ( cd "$HP" && ./release.sh )
fi
CODE_AFTER=$(git -C "$CODE" rev-parse dev)
...
if [ "$BR" = main ]; then
  ...
  # ⭐ **生成物没往前走 = 没有新东西可发布 → 不打号**(判据见文件上方"打号的判据")。
  #    最典型的就是推基号:只动 VERSION,生成物一个字节都不变。
  if [ "$CODE_AFTER" = "$CODE_BEFORE" ]; then
    OWNER_TAG=$(git -C "$CODE" tag --points-at "$CODE_AFTER" --sort=-v:refname \
                | grep -E '^v[0-9]' | head -1)
    echo "[ci] 生成物与上一个号完全一致(hi-proto-code dev 停在 ${CODE_AFTER:0:8})→ **不打号**"
    echo "[ci] 要这份代码就钉 ${OWNER_TAG:-上一个号} —— 它指向的就是这个 commit"
    echo "[ci] done."
    exit 0
  fi
  load_tags
